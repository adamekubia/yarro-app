-- ============================================================
-- Rent ledger for month + arrears from previous months
-- ============================================================
-- Returns: all entries for the selected month PLUS any unpaid/partial
-- entries from previous months (arrears). Arrears show as 'arrears'
-- status so the UI can flag them as worse than current-month overdue.
...
AS $$
  SELECT * FROM (
    ...
    -- Arrears from previous months (unpaid/partial only, since PM signup)
    SELECT
      rl.id AS rent_ledger_id,
      rl.tenant_id,
      t.full_name AS tenant_name,
