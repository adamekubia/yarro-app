...
- Data validation (ticket.data against template schema)
- Timeout detection
...
- Notification throttling (batch by recipient + time window)
- AI message composition

### UI layer owns (not built yet):
- Display template rendering (fill placeholders)
- Bucket grouping (dashboard columns)
- Priority sorting
- Manual override UI (show available overrides per state)
...
- **Template-agnostic:** zero IF statements referencing template slugs
...
- **Timeout-first:** timeout checked before transitions (late data handled by template design)
